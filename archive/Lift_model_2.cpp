...
void transaction_up(int p,int l){           //for transaction of customers at floor
    ...
    while(t[p]<tlif[l]){      //mini next call with changes
        ...
        while (target==p){
            ...
            if(target < p){tar[p][p][0]++; switchh[p][1]=1;}
        }
    }
}
...
void transaction_down(int p,int l){
    ...
    while(t[p]<tlif[l]){      //mini next call with changes
        ...
        while (target==p){
            target = rand()%8;
            if(target < p){tar[p][target][l+1]++;tlif[l]=tlif[l]+0.2;tar[p][p][1]++;}
            if(target > p){tar[p][p][0]++; switchh[p][0]=1;}
        }
    }
}
...
void moveup(int i){
    ...
    print_status();
    while((decide_moveup(i))&&(tlif[i]< minarray(t,8))){
        if (position[i]%2 ==0){
            transaction_up((position[i]/2), i);tlif[i] += 0.3;
        }
        position[i]++;
        lift_direction[i]= 1;
        if (tlif[i]> minarray(t,8)) return;
        if ((position[i]%2)!=0) position[i]++;
        lift_direction[i] = 0;
    }
}
...
void movedown(int i){
    ...
    print_status();
    while((decide_movedown(i))&&(tlif[i]< minarray(t,8))){
...
int decide_up(int i){              //to decide lift for response at floor i
    cout<<"in decide_up call from"<<i<<endl;
    print_status();
    int a[3];
    for(int j=0; j<3;j++){
    ...
    if (minarray(a,3)==a[0]) return 0;
    else if (minarray(a,3)==a[1]) return 1;
    else return 2;
}

int decide_down(int i){
    cout<<"in decide_down call from "<<i<<endl;
    print_status();
    int a[3];
    for(int j= 0; j<3; j++){
        if(lift_direction[j]==0) a[j] = abs(position[j] - 2*i);
    ...
    for(int i=0; i<8;i++){
        if(t[i]==minarray(t,8)){
            ...
            if(target<i){
                ...
                switchh[i][1] = 1;                      //button on that floor
            }
            else{
